...
:usage
echo Usage: run-dmnsim.bat [option]
echo.
echo Options:
echo   --build-only   Compile the project but do not start the simulator.
echo   --clean        Remove the build directory before compiling.
echo   -h, --help     Show this help message.
echo.
echo This script builds the project with javac and runs the desktop
echo simulator entry point on the current JDK.
exit /b 0
...
if /I "%BUILD_ONLY%"=="true" (
	echo Build completed: %CLASSES_DIR%
	goto cleanup_ok
)
